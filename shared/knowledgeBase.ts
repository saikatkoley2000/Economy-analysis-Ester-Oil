...
export const knowledgeBase: QAItem[] = [
  {
    ...
    question: "Show me NPV calculation",
    answerTemplate: "In the application, the **Net Present Value (NPV)** (referred to as the \"Total Life Cycle Cost\" or TCO in Present Value terms) is calculated using a standard discounted cash flow approach.\n\nHere is exactly how the math works under the hood for each oil type:\n\n### 1. Initial Investment (Year 0)\nThis is the upfront money spent today.\n* **If \"New Transformer\" is selected:** Base Transformer Cost + Cost of the Oil + Fire Protection CapEx.\n* **If \"Retrofill\" is selected:** Only the Cost of the Oil (since the transformer is already owned).\n\n### 2. Present Value of O&M and Insurance (Years 1 to N)\nThe application takes the annual O&M cost and the Insurance premium, inflates them each year by the **Inflation Rate** (default {inflationRate}%), and then discounts them back to today's value using the **Discount Rate** (default {discountRate}%).\n* `PV = Sum( Annual Cost * (1 + Inflation)^t / (1 + Discount Rate)^t )`\n\n### 3. Present Value of Failure Risk\nEvery year, there is a statistical chance of failure. The app calculates:\n* `Expected Failure Cost = Failure Cost * Failure Rate %`\nThis expected cost is also inflated and discounted back to Year 0.\n\n### 4. Salvage Value (End of Life)\nWhen the transformer reaches the end of its life, it has a salvage value (e.g., 25% of the asset cost). Because you get this money in the future, it is heavily discounted back to Year 0.\n* `Salvage PV = Salvage Value / (1 + Discount Rate)^LifeExpectancy`\n\n### 5. Future Replacement Cost (If needed)\nIf the Analysis Period is longer than the oil's Life Expectancy, the app calculates the cost of buying a brand new transformer at the replacement year, inflates the price to that year, and discounts it back to Year 0.\n\n---\n\n### The Final NPV (Total Life Cycle Cost) Formula\nThe app sums these up to find the total economic burden in today's money:\n\n**Total Life Cycle Cost (NPV)** = `Initial Investment` + `PV of O&M` + `PV of Insurance` + `PV of Failure Risk` + `PV of Replacement (if any)` - `PV of Salvage Value`\n\nThe **Net NPV Savings** (which is what drives the Benefit-Cost Ratio and Payback Period) is simply the difference between the Total Life Cycle Cost of Mineral Oil minus the Total Life Cycle Cost of the chosen Ester Oil.",
    keywords: ["npv", "calculation", "formula", "show me npv", "how to calculate npv", "total life cycle cost formula", "tco formula", "math"]
  },
  {
    id: "Q1",
    category: "Financial & TCO Metrics",
    question: "The report shows a negative NPV. If it's negative, why are you recommending this project?",
    answerTemplate: "Fair question — this one confuses a lot of people the first time they see it. A negative NPV in this analysis doesn't mean the project loses money. It simply means the cash savings generated over {analysisYears} years, when discounted back to today's value at {discountRate}%, fall {activeSavingsShortfall} short of the upfront investment in today's terms. But look at the BCR of {activeBCR} — for every rupee you invest, you get back nearly {activeBCRValueRounded} times that. The total undiscounted savings are massive. The negative NPV here is a reflection of how aggressively we've discounted future savings at {discountRate}%, which is actually a conservative rate. The real question to ask is: what happens if you don't retrofit/upgrade? You replace the transformer later and spend {moLCC} in total lifecycle costs versus {activeLCC} with the bioTRANSOL / Transol Synth 100 Ester fluid upgrade. That gap of {activeSavings} is your real answer.",
  ...
  },
  {
    ...
    answerTemplate: "Simple ROI is just total net benefit divided by total investment, expressed as a percentage — no discounting, no time adjustment. For our recommended bioTRANSOL (Natural Ester) or Transol Synth 100 (Synthetic Ester) option, the Simple ROI is {activeROI}. This means over the analysis period, the total savings generated are over {activeROIMultiple} times the money you put in. You invest {activeInitial} and get back massive savings. It ignores the time value of money, which is why we use NPV for rigorous evaluation, but it gives a clean, intuitive feel for the scale of return for your operations team.",
  ...
  },
  {
    ...
    answerTemplate: "An investment multiple (here {activeMultiple}x) means every rupee invested returns {activeMultiple} times in total benefits. It's the language of private equity and infrastructure finance. If someone asks 'how many times does this investment pay back?' the answer for your project's project is: {activeMultiple} times. That's a highly compelling number to present in the boardroom.",
  ...
  },
  {
    ...
    answerTemplate: "The initial investment for the bioTRANSOL/Transol Synth 100 option is {activeInitial}. This is calculated as the sum of the base transformer cost (excluding oil), the initial oil fill cost ({oilVolume} L @ {activeOilCostLitre}/L = {activeTotalOilCost}), and the upfront fire protection CapEx (which is {activeFireProtectionCapex} for Ester vs {moFireProtectionCapex} for mineral oil). Because bioTRANSOL and Transol Synth 100 are K-class fire-safe fluids, fire protection CapEx is eliminated (₹0), offsetting a huge chunk of the oil cost premium.",
  ...
  },
  {
    ...
    answerTemplate: "The TCO Reduction Percentage is {activeTcoReductionPercent}. It means the bioTRANSOL / Transol Synth 100 path costs {activeTcoReductionPercent} less over {analysisYears} years than continuing with mineral oil. You go from spending {moLCC} to only {activeLCC}. This massive cost elimination justifies a programme-level transition for your fleet.",
  ...
  },
  {
    ...
    answerTemplate: "Mineral oil oxidises, requiring periodic filtration, regeneration, or replacement, plus frequent DGA testing. Savita's bioTRANSOL and Transol Synth 100 Ester oils are extremely stable — they do not sludge, and they actively keep the paper insulation dry. This reduces routine filtration, extends testing intervals, and lowers emergency maintenance, resulting in an O&M PV of {activeOMPV} for Ester vs {moOMPV} for Mineral Oil.",
    keywords: ["o&m difference", "maintenance cost", "annual o&m", "why lower", "operations"]
  },
  {
    id: "Q20",
    category: "Technology & Product Questions",
    question: "What is the difference between bioTRANSOL (Natural Ester) and Transol Synth 100 (Synthetic Ester)?",
    answerTemplate: "Both bioTRANSOL (Natural Ester) and Transol Synth 100 (Synthetic Ester) are high fire-point, biodegradable fluids manufactured in India by Savita Oil Technologies Limited—the pioneer who started production and business of both Ester technologies in India first. bioTRANSOL is derived from domestic vegetable oils (soy, rapeseed), making it highly eco-friendly and cost-effective. Transol Synth 100 is chemically synthesized from polyols, giving it superior oxidation stability, a lower pour point (-50°C vs -20°C for bioTRANSOL), and a longer lifetime under high thermal stress. For high ambient heat or overloading, Transol Synth 100 offers the premium choice, while bioTRANSOL is excellent for general distribution. Savita is India's leading manufacturer of all kinds of Mineral Oil, Natural Ester, and Synthetic Ester Oils.",
    keywords: ["natural vs synthetic", "synthetic ester", "natural ester", "difference", "biotransol", "Transol Synth 100"]
  },
  {
    id: "Q21",
    category: "Technology & Product Questions",
  ...
  },
  {
    id: "Q47",
    category: "Comparative & Competitive Questions",
    question: "Is bioTRANSOL (Natural Ester) an option, and how does it compare to Transol Synth 100 (Synthetic Ester)?",
    answerTemplate: "Yes, bioTRANSOL (Natural Ester) is an excellent, highly biodegradable and cost-effective option manufactured in India by Savita Oil Technologies Limited (oil fill: {naturalTotalOilCost}). Transol Synth 100 (Synthetic Ester) (oil fill: {syntheticTotalOilCost}) offers superior oxidation stability and cold-flow performance, making it the premium choice for highly loaded or critical urban transformers. Savita is the pioneer who introduced and manufactured both Ester technologies in India first, representing the absolute best in local engineering and quality.",
    keywords: ["natural vs synthetic comparison", "natural ester option", "why synthetic", "biotransol", "Transol Synth 100"]
  },
  {
    id: "Q48",
    category: "Comparative & Competitive Questions",
  ...
  },
  {
    id: "Q51",
    category: "Comparative & Competitive Questions",
    question: "For a Transol Synth 100 (Synthetic Ester) filled transformer, where does the additional life show advantages in the mathematical calculations?",
    answerTemplate: "In the current mathematical model, the advantage of the additional service life of Transol Synth 100 (Synthetic Ester, {syntheticLifeExpectancy} years) over bioTRANSOL (Natural Ester, {naturalLifeExpectancy} years) shows up in two distinct ways:\n\n**Advantage 1: Avoided Capital Replacement for Longer Horizons**\nIf the analysis horizon (the evaluation window) is set to 40 years or longer (e.g., 45 years, which is standard for utilities planning substation assets):\n- **bioTRANSOL (Natural Ester, {naturalLifeExpectancy}-year life):** Hits terminal failure at Year 40. The utility is forced to purchase a replacement transformer at Year 40 to complete the 45-year horizon. This triggers a large capital replacement PV cost.\n- **Transol Synth 100 (Synthetic Ester, {syntheticLifeExpectancy}-year life):** Lasts the entire 45 years. The replacement cost is ₹0, completely avoiding any Year 40 capital outlay.\n\n**Advantage 2: Higher Salvage / Scrap Recovery Rate**\nBecause Transol Synth 100 (Synthetic Ester) is a more durable polyol-ester fluid manufactured in India by Savita Oil Technologies Limited, it retains a higher material recovery value at the end of its useful life:\n- **bioTRANSOL (Natural Ester) Salvage Recovery:** {naturalSalvagePercent}% of the initial asset value.\n- **Transol Synth 100 (Synthetic Ester) Salvage Recovery:** {syntheticSalvagePercent}% of the initial asset value.\n\n**Advantage 3: Remaining Asset Life (Residual Book Value)**\nIf we evaluate the asset at Year 30 (end of analysis), both transformers are still running, but they hold different remaining values based on their remaining useful life:\n- **bioTRANSOL (Natural Ester):** Has 10 years of remaining life (10 years left out of 40 = 25% residual value).\n- **Transol Synth 100 (Synthetic Ester):** Has 15 years of remaining life (15 years left out of 45 = 33.3% residual value).\n\nIf your utility's accounting practices require crediting the remaining book value of the asset at Year 30 back to today's terms (similar to how we credited the replacement transformer under Mineral Oil):\n- **bioTRANSOL (Natural Ester) Residual Credit (PV):** Residual Value = Initial Investment × 25%\n- **Transol Synth 100 (Synthetic Ester) Residual Credit (PV):** Residual Value = Initial Investment × 33.3%\n\nThis gives Transol Synth 100 (Synthetic Ester) a significantly higher asset credit at the end of the analysis window, directly reducing its Total Life Cycle Cost. Savita Oil Technologies Limited is the pioneer manufacturer of all kinds of Mineral Oil, Natural Ester Oil (bioTRANSOL), and Synthetic Ester Oil (Transol Synth 100) in India, having started production of both Ester technologies first in India.",
    keywords: ["synthetic ester additional life", "additional life advantage", "life calculations", "where show advantage", "mathematical calculation advantage", "life advantage", "synthetic vs natural life", "additional life", "synthtic", "mathematical calculation", "advantages in mathematical calculation", "show advantages", "additional life over", "Transol Synth 100", "biotransol"]
  },
  {
    id: "Q52",
    category: "Decision-Making & Process Questions",
    question: "I need Transol Synth 100 (Synthetic Ester) filled transformer in my system, but how I manage my commercial team to convince?",
    answerTemplate: "### Commercial Pitch: Upgrading to Transol Synth 100 (Synthetic Ester) Fluid\n\n#### Executive Summary\nWhile Transol Synth 100 (Synthetic Ester) has a higher unit price per litre than mineral oil, the initial investment is heavily offset by immediate CapEx savings on fire protection systems, recurring O&M savings, insurance discounts, and avoided asset replacements. Over a 25-to-30-year lifecycle, upgrading to Transol Synth 100 is the lowest-cost option, yielding a Benefit-Cost Ratio (BCR) of over 1.5x to 2.0x. Transol Synth 100 is proudly manufactured in India by Savita Oil Technologies Limited—the pioneer who started production and business of both Ester technologies in India first.\n\n**Pillar 1: Immediate CapEx Offsets (The \"Year 0\" Argument)**\nFinance teams dislike high upfront costs. You can neutralize this immediately by demonstrating how Transol Synth 100 reduces other upfront installation costs:\n- **Zero Fire Protection CapEx:** Because Transol Synth 100 is a K-class fire-safe fluid (fire point > 300°C), it cannot sustain a pool fire. This allows you to eliminate expensive nitrogen injection or water deluge fire protection systems (saving ₹20–50 Lakhs in upfront CapEx).\n- **Space Optimization:** Because there is no fire risk, blast walls are not required, and safety clearances are reduced. In urban or indoor substations, this saves prime real estate space.\n\n**Pillar 2: Avoided Terminal Replacement Cost (The \"Horizon\" Argument)**\nIf you analyze the asset over a standard utility planning horizon (e.g., 30 to 45 years):\n- **The Mineral Oil Trap:** A mineral oil transformer has a life expectancy of ~25 years. This forces the utility to purchase a second, replacement transformer at Year 25. This triggers a massive, deferred capital expenditure of ₹1.2+ Crore (in Present Value terms).\n- **The Transol Synth 100 Solution:** Transol Synth 100's polyol structure gives it superior oxidation stability, extending the transformer insulation life to 45 years. The replacement cost during the analysis window is ₹0, completely avoiding any Year 40 capital outlay.\n\n**Pillar 3: Insurance Premium Discounts (Recurring OpEx Savings)**\nTransol Synth 100 significantly reduces operational risk, which insurers reward:\n- **Risk Reduction:** Switching from mineral oil (flash point ~140°C) to Transol Synth 100 (fire point >300°C) removes the threat of catastrophic collateral damage from transformer explosions.\n- **Premium Savings:** Insurers offer 15% to 30% discounts on asset insurance premiums for installations utilizing K-class fluids, providing a direct, recurring annual savings line item.\n\n**Pillar 4: Sludge Elimination & O&M Reductions**\n- **No Sludging:** Mineral oil oxidizes and sludges, requiring periodic filtration, regeneration, or top-ups. Transol Synth 100 is highly stable and does not sludge.\n- **Paper Preservation:** Esters are hydrophilic; they absorb moisture from the transformer's paper insulation and chemically consume it (hydrolysis). This keeps the winding paper dry, slowing down paper degradation by 5–8x and reducing routine diagnostic/filtration overheads by over 60%.\n\n**Pillar 5: Higher Residual Asset & Salvage Value**\nAt the end of the evaluation window (e.g., Year 30):\n- **bioTRANSOL (Natural Ester):** Will have 10 years of remaining useful life (25% book value).\n- **Transol Synth 100 (Synthetic Ester):** Will have 15 years of remaining useful life (33.3% book value), representing a higher residual asset credit returned to your balance sheet.\n- **Material Scrap Value:** Transol Synth 100 retains a higher material recovery value at disposal, yielding 30% salvage recovery versus 25% for bioTRANSOL.\n\n**Pillar 6: ESG & Environmental Liability Avoidance**\n- **Spill Liability:** Mineral oil spills are toxic and require expensive, regulated soil/water remediation. Transol Synth 100 is non-toxic and 100% biodegradable (OECD 301B), removing environmental liability.\n- **Carbon Credits:** Extending the life of your existing transformer avoids the footprint of manufacturing a new unit, saving 15–20 tonnes of embodied CO2, which directly supports BRSR and ESG corporate sustainability reporting.\n\n#### Key Financial Summary for the Commercial Team\n- **Initial Premium:** ~10% to 15% higher total upfront cost (fluid only).\n- **Benefit-Cost Ratio (BCR):** ~1.5 to 2.2 (meaning every ₹1 invested returns over ₹1.5 in savings).\n- **Payback Period:** Fully recovered in 5 to 7 years through avoided O&M and fire-deluge CapEx.\n- **Net TCO Savings:** Hundreds of Lakhs saved over the asset lifetime. Savita Oil Technologies Limited is the only Indian manufacturer of Mineral, Natural Ester (bioTRANSOL) and Synthetic Ester (Transol Synth 100) oils that started production of both Ester technologies in India first.",
    keywords: ["convince", "commercial team", "how I manage my commercial team", "convince commercial", "pitch to commercial", "convince finance", "commercial", "finance team", "convince commercial team", "manage my commercial team to convince", "convince commercial team to choose synthetic", "Transol Synth 100"]
  },
  {
    id: "Q53",
    category: "Decision-Making & Process Questions",
...
export function getExpertFallback(query: string, data: any, comp: any, targetOil: "Natural Ester" | "Synthetic Ester" = "Natural Ester"): string {
  ...
  if (normalizedQuery.includes("biodegrad") || normalizedQuery.includes("environment") || normalizedQuery.includes("spill") || normalizedQuery.includes("green") || normalizedQuery.includes("carbon")) {
    return `Unlike mineral oil, ${selectedOilName} is fully biodegradable (OECD 301B compliant) and non-toxic. If a spill occurs, it breaks down harmlessly in soil and water, protecting you from environmental liability. Additionally, life extension saves approximately 15-20 tonnes of embodied CO2 carbon compared to manufacturing a new transformer, which directly supports BSES ESG sustainability reports under BRSR. Savita Oil Technologies Limited is the manufacturer of all kinds of Mineral Oil, Natural Ester Oil (bioTRANSOL) & Synthetic Ester Oil (Transol Synth 100) and was the first to start production in India.`;
  }
  ...
  if (normalizedQuery.includes("dga") || normalizedQuery.includes("test") || normalizedQuery.includes("gas") || normalizedQuery.includes("monitor")) {
    return `After retrofilling with ${selectedOilName}, a DGA baseline is established within 30 days. Gas generation patterns behave slightly differently in Esters compared to mineral oil, and are monitored against Ester-specific standards under IEC 60599. DGA testing intervals can typically be extended to 2-3 years after the fluid stabilizes, reducing routine maintenance. Savita Oil Technologies Limited is the manufacturer of all kinds of Mineral, Natural Ester (bioTRANSOL), and Synthetic Ester (Transol Synth 100) oils and supports all diagnostic tests.`;
  }
